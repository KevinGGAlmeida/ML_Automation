...
class Request:
    ...
    def get_list_of_products(self):
        self.response = self.request.get(
            ...
            {self.product_name}_NoIndex_True_PRICE*DESC_True"""
        ).text

    def filtering_data(self):
        soup = BeautifulSoup(self.response, "html.parser")
        soup_json = json.loads(
            soup.find(
        ...
        for values in range(1, len(soup_json["@graph"]) - 1):
            self.filtered_list.append(
                {
                    "Numero": values,
                    "Produto": soup_json["@graph"][values]["name"],
                    "Preco": soup_json["@graph"][values]["offers"][
                        "price"
                    ],
                    "link": soup_json["@graph"][values]["offers"][
                        "url"
                    ],
                }
            )
